...
static void* ExternalTask(void* arg)
{
...
#endif
#if defined(WIN32_USE_UART) && defined(_WIN32)	
			UartSend(WIN32_UART_PORT, outDataBuf, MAX_OUTDATA_SIZE);
			printf("UART Send\r\n");
#endif
		}
...
#endif
#if defined(WIN32_USE_UART) && defined(_WIN32)	
		readLen = UartReceive(WIN32_UART_PORT, inDataBuf, EXTERNAL_BUFFER_SIZE);
		if (readLen)
		{
			int i;
			printf("UART Rec %d>>", readLen);
			for (i = 0; i < readLen; i++)
				printf("[%x]-", inDataBuf[i]);
			printf("\r\n");
		}
#endif
	...
	extOutQueue = -1;
#if defined(WIN32_USE_UART) && defined(_WIN32)	
	ComExit(WIN32_UART_PORT, WIN32_UART_BUNDRATE);
#endif
	return NULL;
}
...
#endif
#if defined(WIN32_USE_UART) && defined(_WIN32)	
	ComInit(WIN32_UART_PORT, WIN32_UART_BUNDRATE);
#endif
